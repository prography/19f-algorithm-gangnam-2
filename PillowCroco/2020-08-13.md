...
public:
    int maxProfitAssignment(vector<int>& difficulty, vector<int>& profit, vector<int>& worker) {
        vector<pii> dp(difficulty.size(),0);
        ...
        int max_p = 0;
        for(pii a: dp){
            max_p = max(max_p, -a.second);
            a.second = max_p;
        }

        // worker gets the most profit job by binary search. mlogn
        // current.d <= worker[i] < next.d : then ans+=current.p
        int ans = 0;
        for(int d: worker){
            int left=0, right = dp.size()-1, mid = left+ (right-left)/2;
            while(true){
                if(d<dp[mid].first){
                    right = mid;
                    mid = left + (mid-left)/2;
